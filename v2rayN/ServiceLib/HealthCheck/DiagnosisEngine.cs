...
public interface IDiagnosisRule
{
    ...
    int Priority { get; }
    /// <summary>Optional auto-fix actions recommended by this rule.</summary>
    IReadOnlyList<HealthCheckFixId> FixIds { get; }
}
...
public static class DiagnosisEngine
{
    ...
    {
        var isZh = ResolveZh(locale);
        var findings = new List<string>();
        foreach (var rule in MatchedRules(report))
        {
    ...
    public static List<HealthCheckFixAction> CollectFixes(HealthCheckReport report)
    {
        var ids = new HashSet<HealthCheckFixId>();
        foreach (var rule in MatchedRules(report))
        {
            foreach (var id in rule.FixIds)
            {
                ids.Add(id);
            }
        }
        return ids
            .Select(HealthCheckAutoFixService.Describe)
            .Where(a => a != null)
            .Cast<HealthCheckFixAction>()
            .OrderBy(a => a.Id)
            .ToList();
    }

    private static IEnumerable<IDiagnosisRule> MatchedRules(HealthCheckReport report) =>
        Rules.Where(r => r.Matches(report)).OrderBy(r => r.Priority).Take(5);

    private static bool ResolveZh(string? locale)
    {
...
public class TunNotfoundRule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [HealthCheckFixId.RebootAsAdmin];
...
public class TunNoIpRule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [HealthCheckFixId.ReloadCore];
...
public class TunNoRouteRule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [HealthCheckFixId.EnableAutoRouteStrictRoute, HealthCheckFixId.ReloadCore];
...
public class NonTunModeRule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [HealthCheckFixId.EnableTun];
...
public class RoutingLoopRule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [HealthCheckFixId.ExcludeServerIpFromTun, HealthCheckFixId.ReloadCore];
...
public class OutboundTcpFailedRule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [];
...
public class WebsiteAllFailedBut204Rule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [HealthCheckFixId.SetMtu1280, HealthCheckFixId.ReloadCore];
...
public class CrossLayerTunBypassRule : IDiagnosisRule
{
    ...
    public IReadOnlyList<HealthCheckFixId> FixIds => [HealthCheckFixId.EnableTun, HealthCheckFixId.EnableAutoRouteStrictRoute, HealthCheckFixId.ReloadCore];
