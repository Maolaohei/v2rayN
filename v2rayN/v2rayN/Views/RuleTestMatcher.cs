...
public class GeoDataManager
{
    ...
    private static string? ParseDomainEntry(ByteString domainData)
    {
        ...
        // v2ray Entry.Type: 0=Plain, 1=Regex, 2=Domain, 3=Full
        return type switch
        {
            0 => $"plain:{value}",
            1 => $"regex:{value}",
    ...
    public static bool IsDomainInGeosite(string countryCode, string domain)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            else if (rule.StartsWith("regex:"))
            {
                try
                {
                    if (Regex.IsMatch(domain, rule["regex:".Length..], RegexOptions.IgnoreCase)) return true;
                }
                catch { }
            }
        }
    ...
    private static bool IsInCidr(byte[] targetBytes, byte[] networkBytes, int prefixLen)
    {
        ...
        if (targetBytes.Length != 4 || networkBytes.Length != 4) return false;
...
public static class RuleTestMatcher
{
    public static List<RuleTestResult> TestAllRules(string input, List<RulesItem> rules)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            if (IsDomainMatch(inputLower, rule.Domain))
                matchedFields.Add("domain");

            if (IsIpMatch(input, rule.Ip))
                matchedFields.Add("ip");

            if (IsPortMatch(input, rule.Port))
                matchedFields.Add("port");

            if (IsProcessMatch(inputLower, rule.Process))
                matchedFields.Add("process");

            var matched = matchedFields.Count > 0;
    ...
    public static bool IsDomainMatch(string inputLower, List<string>? domains)
    {
        ...
        foreach (var rule in domains)
        {
            ...
            if (r.StartsWith("regexp:"))
            {
                try
                {
                    if (Regex.IsMatch(inputLower, r["regexp:".Length..], RegexOptions.IgnoreCase))
                        return true;
                }
                catch { }
                continue;
    ...
    public static bool IsProcessMatch(string inputLower, List<string>? processes)
    {
        ...
        foreach (var rule in processes)
        {
            ...
            if (inputLower == r || inputLower.EndsWith("\\" + r) || inputLower.Contains(r))
                return true;
        }
